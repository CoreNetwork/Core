...
public class GriefPreventionHandler {
	...
	public static void secure(Location corner, List<Location> chestSubclaims, int xSize, int zSize, Integer claimPermission)
	{
		...
		{
			CLog.severe("Creation of GriefPrevention claim at " + corner.getBlockX() + " " + corner.getBlockZ() + " failed! Please review that location manually (" + bigClaimResult.succeeded.toString() + ")");
			return;
		}
		
		Claim bigClaim = bigClaimResult.claim;

		CLog.debug("Creating claim " + bigClaim.getID());
		...
		{			
			for (Location subClaimLoc : chestSubclaims)
			{
				...
				{
					CLog.severe("Creation of GriefPrevention sub claim at " + subClaimLoc.getBlockX() + " " + subClaimLoc.getBlockZ() + " failed! Please review that location manually (" + chestClaimResult.succeeded.toString() + ")");
					continue;
				}
